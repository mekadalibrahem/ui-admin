...
<section class="bg-white container mx-auto md:w-5/5 lg:w-4/5   mt-8 rounded-lg  dark:bg-gray-900">
    <div class="w-full px-4 py-8 mx-auto ">
        <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Permissions : </h2>
        @if ($status !== [])
                    ...
                    @foreach ($permissions as $permission)
                        <tr class="border-b dark:border-gray-700" wire:key="{{ $permission->id }}">
                            ...
                            <td class="px-4 py-3 flex flex-row md:justify-center gap-2">
                                <x-uiadmin.button.primary wire:click="" type="button" class="w-min">
                                    <x-uiadmin.svg.arrow-up />
                                </x-uiadmin.button.primary>
                                <x-uiadmin.button.danger type="button" wire:click="delete({{ $permission->id }})"
        ...
        {{ $permissions->links('components.uiadmin.pagination') }}
